...
type SpreadOverrideModalProps = {
  ...
  onSpreadOverrideSaved: (row: Quote) => Promise<void>
...
export function SpreadOverrideModal({
  ...
  return (
    <Modal
      ...
      width={400}
      centered
      cancelButtonProps={{ disabled: saveSpreadOverrides?.isPending, tabIndex: 1 }}
      onCancel={() => {
        setIsSpreadOverrideModalOpen(false)
      }}
      okButtonProps={{
        disabled: !isDefinedAndNotNull(overrideValue),
        loading: saveSpreadOverrides?.isPending,
        tabIndex: 0,
      }}
      ...
      onOk={() => {
        applyOverride()
          .then(() => {
            if (selectedSpreadOverrideRow) {
              return onSpreadOverrideSaved(selectedSpreadOverrideRow)
            }
          })
          .catch(() => {
            // mutateAsync onError already shows toast via toastApiError
          })
      }}
    >
      <Vertical>
