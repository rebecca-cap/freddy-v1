...
import { BulkMarketMoveOverrideEditor } from '@modules/PricingEngine/QuoteBook/Components/Grid/Components/cellEditors/BulkMarketMoveOverrideEditor'
import { BulkPriceEditor } from '@modules/PricingEngine/QuoteBook/Components/Grid/Components/cellEditors/BulkPriceEditor'
...
export const ProposedColumns = ({
  ...
  const isUsingFreight = !!metadata?.ShowFreightColumns
  const isUsingTax = !!metadata?.ShowTaxColumns
  const isEndOfDay =
    ...
    return [
      {
        ...
        children: [
          ...
          ...(isUsingMarketMove
            ? [
                MarketMoveColumn(setIsMarketMoveBreakdownAndValuationDrawerOpen, setSelectedValuationRow),
                MarketMoveOverrideColumn(originalRows, canWrite),
              ]
            : []),
          ...(isUsingFreight ? [FreightColumn()] : []),
          ...(isUsingTax ? [TaxColumn()] : []),
...
const ProposedCost = ({ setIsFormulaBreakdownAndValuationDrawerOpen, setSelectedValuationId }) => ({
  field: 'Cost',
...
const ProposedStrategyBase = ({ originalRows }) => ({
  ...
  cellRenderer: (params) => {
    ...
    return (
      <Button style={{ textAlign: 'left', padding: 0, margin: 0 }} type='link'>
        <Texto>
          <Horizontal verticalCenter gap='0.5rem'>
...
const EODProposedDiff = ({ canWrite, originalRows }) => ({
  ...
  cellStyle: (params) => {
    ...
    if (value > 0) style = { color: 'var(--theme-success)', fontWeight: 'bold' }
...
const MiddayProposedDiff = ({ canWrite, originalRows }) => ({
  ...getStrategyColumnDef({ field: 'Adjustment', propertyName: 'QuoteStrategyDiffName' }),
  filter: 'agNumberColumnFilter',
  editable: (params) => canWrite && !params?.data?.SpreadParentMappingId,
  isBulkEditable: canWrite,
...
const ProposedPrice = ({ canWrite, originalRows }) => ({
  editable: (params) => canWrite && !params?.data?.SpreadParentMappingId,
  isBulkEditable: canWrite,
  ...
  valueGetter: (params) => {
    ...
    if (!row) return null
    const adjustmentValue = row.SpreadOverride ?? row.Adjustment
    const basePrice = isSpreadRow(params)
      ? (row.StrategyBase?.Value ?? 0) + (adjustmentValue ?? 0)
      : row.ProposedPrice
    if (basePrice == null) return basePrice
    const freight = row.UsesFreight ? (row.FreightValue ?? 0) : 0
    const tax = row.UsesTax ? (row.TaxValue ?? 0) : 0
    return basePrice + freight + tax
  },
})
const ProposedPriceDelta = ({ originalRows }) => ({
  editable: false,
...
function GetProposedMargin(params) {
  ...
  const proposedPrice = isSpreadRow(params)
    ? row?.StrategyBase?.Value + adjustmentValue
    : row?.ProposedPrice
  ...
  const freightTaxExclusion = row?.ExcludeFreightAndTaxFromMargin
    ? (row?.UsesFreight ? row?.FreightValue ?? 0 : 0) + (row?.UsesTax ? row?.TaxValue ?? 0 : 0)
    : 0
  return proposedPrice - freightTaxExclusion - cost
}
...
function GetTemperatureAdjustedMargin(params) {
  ...
  const freightTaxExclusion = rowData?.ExcludeFreightAndTaxFromMargin
    ? (rowData?.UsesFreight ? rowData?.FreightValue ?? 0 : 0) + (rowData?.UsesTax ? rowData?.TaxValue ?? 0 : 0)
    : 0
  const priceForMargin = proposedPrice - freightTaxExclusion

  const useMultiplication = rowData?.TCIIsMultiplication !== false
  const adjustedPrice = useMultiplication ? priceForMargin * tciAdjustment : priceForMargin / tciAdjustment

  // temp adjusted margin is the margin computed when cost is adjusted for temperature
  return adjustedPrice - cost
}

const ProposedTemperatureAdjustedMargin = () => ({
  minWidth: 90,
...
const MarketMoveOverrideColumn = (originalRows: Quote[] | undefined, canWrite: boolean) => ({
