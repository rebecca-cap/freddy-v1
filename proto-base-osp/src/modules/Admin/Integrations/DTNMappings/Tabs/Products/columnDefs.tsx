...
export function dtnProductColumnDefs({ metadata, canWrite }: IGetColumnDefProps) {
  return [
    {
      editable: false,
      field: 'SourceValue',
    ...
    },
    {
      ...
      valueGetter: (params) =>
        metadata?.Products?.find((option) => option.Value?.toString() === params?.data?.TargetProductId?.toString())
          ?.Text,
      cellRenderer: ({ value }) => {
        if (value) {
          return (
