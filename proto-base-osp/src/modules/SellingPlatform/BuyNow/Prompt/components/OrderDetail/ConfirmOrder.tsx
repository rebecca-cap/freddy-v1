...
export function ConfirmOrder({ form }: { form: FormInstance }) {
  const { pendingTrade, tasMode, isDateOverrideActive, currentFromDate, currentToDate, selectedItemMeta } =
    usePromptContext()
  const confirmFromDate = pendingTrade?.OverrideStartDate ?? currentFromDate
  const confirmToDate = pendingTrade?.OverrideEndDate ?? currentToDate
  const tzAlias = selectedItemMeta?.TimeZoneAlias || serverTimeZoneAlias
          ...
          <Horizontal className='mx-4 my-2 justify-sb bg-1 py-2 px-3 bordered'>
            <Vertical>
              <Horizontal className='justify-sb py-2'>
                ...
                <Texto category='h6'>
                  {dayjs(confirmFromDate).format(dateFormat.SHORT_DATE_YEAR_TIME_V2)}
                  {tzAlias ? ` ${tzAlias}` : ''}
                </Texto>
              </Horizontal>
              ...
              <Horizontal className='justify-sb py-2'>
                ...
                <Texto category='h6'>
                  {dayjs(confirmToDate).format(dateFormat.SHORT_DATE_YEAR_TIME_V2)}
                  {tzAlias ? ` ${tzAlias}` : ''}
                </Texto>
              </Horizontal>
            </Vertical>
