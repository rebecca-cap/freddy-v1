  ...
  const getQuantityFieldStatus = useMemo(
    () => (getStatus?: boolean) => {
      ...
      if (value && value > constraints.MaxVolume) {
        return getStatus
          ? 'error'
          : `Volume cannot exceed max of ${fmt.decimal(constraints.MaxVolume, 0)} ${uomSymbol}(s)`
      }
        ...
        rules={[
          {
            validator: (_, value) => {
              ...
              if (value > constraints.MaxVolume) {
                return Promise.reject(
                  new Error(`Volume cannot exceed max of ${fmt.decimal(constraints.MaxVolume, 0)} ${uomSymbol}(s)`)
                )
              }
