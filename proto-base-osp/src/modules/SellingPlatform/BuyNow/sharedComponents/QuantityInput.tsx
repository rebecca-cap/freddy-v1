...
export function QuantityInput({
  ...
  const uomSymbol = selectedItemMeta?.UnitOfMeasureSymbol ?? defaultUnitOfMeasureSymbol
  const getQuantityFieldStatus = useMemo(
    () => (getStatus?: boolean) => {
      ...
      const effectiveMax = selectedItemMeta?.SpecialOfferData?.EffectiveMaxPerOrder ?? constraints?.MaxVolume
      ...
      if (value && value < constraints.MinVolume) {
        return getStatus ? 'error' : `Volume must be at least ${fmt.decimal(constraints.MinVolume, 0)} ${uomSymbol}(s)`
      }
      if (value && value > effectiveMax) {
        return getStatus ? 'error' : `Volume cannot exceed max of ${fmt.decimal(effectiveMax, 0)} ${uomSymbol}(s)`
      }
    ...
    [form.getFieldValue('Quantity'), uomSymbol]
  )
    ...
    >
        ...
        rules={[
          {
            validator: (_, value) => {
              ...
              if (value > effectiveMax) {
                return Promise.reject(
                  new Error(`Volume cannot exceed max of ${fmt.decimal(effectiveMax, 0)} ${uomSymbol}(s)`)
                )
              }
      ...
      >
        <InputNumber
          ...
          className='round-border volume-input-dark'
          size='middle'
          onChange={(value) => {
