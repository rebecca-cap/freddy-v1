...
export function ConfirmOrder({ form }: { form: FormInstance }) {
  const { pendingTrade, tasMode, selectedItemMeta, selectedOffer, isDateOverrideActive } = useOffersContext()
  ...
  const selectedAdditionalProducts = pendingTrade?.SelectedItems?.filter(
      ...
      <Vertical>
        ...
        >
          ...
          <Texto className='test-ConfirmQuantity' category='h3' appearance='white'>
            {fmt.decimal(pendingTrade?.Quantity ?? 0, 0)}
          </Texto>
        </Horizontal>
        <Horizontal
        ...
        >
          ...
          <Vertical flex={1} verticalCenter>
            <Texto weight={900} category='h5' appearance={tasMode ? 'warning' : 'white'} style={{ textAlign: 'right' }}>
              {formatPricePerUnit(pendingTrade?.Price ?? 0, {
                currencyName: selectedOffer?.CurrencySymbol,
                uomSymbol: selectedOffer?.UnitOfMeasureSymbol,
              })}
            </Texto>
          </Vertical>
        </Horizontal>
        {pendingTrade?.IsInternalUser && !tasMode && pendingTrade?.Type === 'market' && (
          <Horizontal className='mx-4 my-2 justify-sb bg-1 py-2 px-3 bordered'>
            ...
            <Texto category='h6' className='test-ConfirmIndexPrice'>
              {fmt.currency(pendingTrade?.IndexPrice ?? 0)}
            </Texto>
          </Horizontal>
        )}
              ...
              {!!pendingTrade?.DestinationStates?.length && (
                <Horizontal className='justify-sb py-2'>
                  ...
                  {pendingTrade?.DestinationStates.map((location) => {
                    return <Texto key={location?.LocationName}>{location?.LocationName}</Texto>
                  })}
                </Horizontal>
              )}
              {!!pendingTrade?.LoadingNumbers?.length && (
                <Horizontal className='justify-sb py-2'>
                  ...
                  {pendingTrade?.LoadingNumbers.map((loadingNumber) => {
                    return <Texto key={loadingNumber?.Display}>{loadingNumber?.Display}</Texto>
                  })}
                </Horizontal>
              )}
              {!!pendingTrade?.LiftingLocationIds?.length && (
                <Horizontal className='justify-sb py-2'>
                  ...
                  {pendingTrade?.LiftingLocations.map((location) => (
                    <Texto category='p2' key={location}>
                      {location}
                    </Texto>
                  ))}
                  )
        ...
        {(pendingTrade?.IsInternalUser || selectedItemMeta?.SpecialOfferData) && selectedItemMeta?.ShowDateOverrideFields && (
          <Horizontal className='mx-4 my-2 justify-sb bg-1 py-2 px-3 bordered'>
            <Vertical>
              <Horizontal className='justify-sb py-2'>
                <Texto style={{ color: 'var(--theme-option)' }} category='p2'>
                ...
                <Texto category='h6'>
                  {dayjs(pendingTrade?.OverrideStartDate).format(dateFormat.SHORT_DATE_YEAR_TIME_V2)}
                </Texto>
              </Horizontal>
              ...
              <Horizontal className='justify-sb py-2'>
                ...
                <Texto category='h6'>
                  {dayjs(pendingTrade?.OverrideEndDate).format(dateFormat.SHORT_DATE_YEAR_TIME_V2)}
                </Texto>
              </Horizontal>
            </Vertical>
          </Horizontal>
