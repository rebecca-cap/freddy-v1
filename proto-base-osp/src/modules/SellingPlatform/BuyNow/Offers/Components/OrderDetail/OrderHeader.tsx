...
export function OrderHeader({ form, showOrderTimer = true, showTimeRemaining = true }) {
  const { selectedItemMeta, selectedOffer, tradeTimer, setIsPriceExpired } = useOffersContext()
  ...
  const tzAlias = selectedItemMeta?.SpecialOfferData?.TimeZoneAlias || serverTimeZoneAlias
      ...
      <Vertical style={{ flex: 1 }} className='justify-end'>
        ...
        <Texto category='h5' appearance='white' weight={500} align='right'>
          {isBid ? (
            ...
            <b>
              {formatPricePerUnit(selectedItemMeta?.Price ?? 0, {
        ...
        {showTimeRemaining && (
          <Horizontal justifyContent='flex-end'>
            <Texto
              ...
              style={
                isWithin24Hours(offerTimespan)
                  ? { backgroundColor: 'var(--bg-1)', borderRadius: 12, padding: '2px 10px' }
                  : undefined
              }
            >
