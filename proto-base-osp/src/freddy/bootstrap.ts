...
if (enabled) {
  ...
  // OSP persona: default the theme to OSP (navy/cyan) on first load. excalibrr's
  // ThemeContextProvider reads TYPE_OF_THEME and would otherwise seed LIGHT_MODE.
  // Only set when missing so a user's later theme switch persists.
  if (!localStorage.getItem('TYPE_OF_THEME')) {
    localStorage.setItem('TYPE_OF_THEME', 'OSP')
  }
