...
import {
  locationsMetadataFixture,
  locationsReadFixture,
  locationHierarchyListFixture,
} from './locations'
import {
  productsMetadataFixture,
  productsReadFixture,
  productHierarchyListFixture,
} from './products'
...
import { orderEntryDataFixture, forwardPricesFixture, submitOrderFixture } from './orderEntry'
import {
...
import {
  ...
  authorizedCounterPartyIdsFixture,
  createSpecialOfferFixture,
} from './specialOffers'
import {
...
export const fixtures: Record<string, (path?: string, init?: any) => Promise<unknown>> = {
  ...
  // CommandCenterPage seeds hierarchy type ids from [0].Key of these bare arrays.
  'ReferenceData/Hierarchy/Location/List': async () => locationHierarchyListFixture,
  'ReferenceData/Hierarchy/Product/List': async () => productHierarchyListFixture,

  // /BuyNow/Forward + /BuyNow/Prompt order entry (usePrompt + Forwards)
  'MarketPlatform/OrderEntry/GetOrderEntryData': async () => orderEntryDataFixture,
  'MarketPlatform/OrderEntry/GetItemsAvailableForOrder': async () => {
    await new Promise((r) => setTimeout(r, 600))
    return forwardPricesFixture
  },
  // Confirm step → "Order #N Saved Successfully" + loading number.
  'MarketPlatform/OrderEntry/SubmitOrder': async () => submitOrderFixture,
  ...
  // /SpecialOffers admin (Manage Offers). Prod's typed hook calls GetMetaData
  // (capital D); the older raw-fetch path used GetMetadata. Serve both casings.
  ...
  'MarketPlatform/SpecialOfferAdmin/GetMetaData': async () => specialOffersMetadataFixture,
  // Create New Offer wizard: step-4 authorization join + submit.
  'MarketPlatform/SpecialOfferAdmin/GetAuthorizedCounterPartyIds': async () =>
    authorizedCounterPartyIdsFixture,
  'MarketPlatform/SpecialOfferAdmin/CreateSpecialOffer': async () => createSpecialOfferFixture,
  ...
  // /OnlineOrders — prod replaced the EntityReport dispatch with a dedicated
  // typed endpoint (useOnlineOrdersTyped → GetOrders). Reuse the same rich rows.
  // (The page reads response.Data; getMetaData is not called by OnlineOrdersPage.)
  'MarketPlatform/OnlineOrders/GetOrders': async () => onlineOrdersReadFixture,
