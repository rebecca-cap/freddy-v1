...
import { ContractDetails, ContractManagementMetadata, Detail, FormulaVariable } from '@modules/ContractManagement/api/types.schema'
import { contractQueryKeys, useContractsTyped } from '@modules/ContractManagement/api/useContractsTyped'
import { ValuationData } from '@modules/ContractManagement/components/DetailsView/api/types.schema'
import { useContractDetailValuationTyped } from '@modules/ContractManagement/components/DetailsView/api/useContractDetailValuationTyped'
...
/**
 * Keeps `?detailId` in sync with the active tab. Extracted from the provider into
 * its own component so the state→URL effect only re-renders this leaf when it
 * fires `setSearchParams`, instead of forcing the whole provider body — and its
 * `value` object — to recompute on every URL write.
 *
 * Reads `window.location.search` directly (not React Router's `searchParams`)
 * so the effect doesn't take a dep on a value that changes as a side effect of
 * the effect itself.
 */
function DetailIdUrlSync() {
  const { activeTabId, details, pageMode } = useContractManagementContext()
  ...
  useEffect(() => {
    ...
    const currentDetailId = current.get('detailId')
    let nextDetailId: string | null = null

    if (activeTabId && activeTabId !== '0' && !activeTabId.includes('duplicate')) {
      const activeDetail = details?.find((d) => d.LocalTradeEntryDetailId === activeTabId)
      if (activeDetail?.TradeEntryDetailId) {
        nextDetailId = activeDetail.TradeEntryDetailId.toString()
      }
    }

    // ?expandFormula is a one-shot intent consumed by ProvisionManager when a detail tab is
    // open. If no detail tab is active (All Details / no tab / draft / duplicate), strip it
    // here — the flag is meaningless without a formula to expand.
    const isOnNonDetailView = !activeTabId || activeTabId === '0' || activeTabId.includes('duplicate')
    const shouldStripExpandFormula = isOnNonDetailView && current.has('expandFormula')

    if (currentDetailId === nextDetailId && !shouldStripExpandFormula) return

    setSearchParams(
      (prev) => {
        const next = new URLSearchParams(prev)
        if (nextDetailId) next.set('detailId', nextDetailId)
        else next.delete('detailId')
        if (shouldStripExpandFormula) next.delete('expandFormula')
        return next
      },
      { replace: true }
    )
  }, [activeTabId, details, pageMode, setSearchParams])
...
type ContractManagement = {
  ...
  saveDetail: (detail: Detail) => void
  saveHeader: (header: Partial<ContractDetails>) => void
...
export const ContractManagementProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const { useALLContractManagementData } = useContractsTyped()
  ...
  const { useContractDetails, submitContract } = useContractsTyped()
  ...
  const { getContractDetailValuation, getContractValuation } = useContractDetailValuationTyped()
  const {
  ...
  useEffect(() => {
    if (contractValuationData?.Data && contractValuationData.Data.length > 0) {
      setValuationData(contractValuationData.Data)
    }
  }, [contractValuationData])

  function checkAndSetValuationData() {
    if (!valuationData.length && contractDetailValuationData?.Data) setValuationData(contractDetailValuationData.Data)
  ...
  useEffect(() => {
    if (contractDetailValuationData?.Data && contractDetailValuationData.Data.length > 0) {
  ...
  useEffect(() => {
    if (contract && metadata && metadata?.TradeInstrumentList) {
      ...
      // Apply ?detailId=<TradeEntryDetailId> deep-link by opening that detail's tab.
      // Invalid or stale ids fall through silently and the user lands on All Details.
      // Read window.location directly here (not the React Router searchParams) so this
      // effect only re-runs on contract/metadata changes, not on every URL update.
      const urlDetailId = new URLSearchParams(window.location.search).get('detailId')
      const fixedDetails = contract.Details.map((detail) => {
        const fixed = fixDetails(detail, metadata)
        if (urlDetailId && fixed.TradeEntryDetailId?.toString() === urlDetailId) {
          return { ...fixed, isOpen: true }
        }
        return fixed
      })
      const targetDetail = urlDetailId
        ? fixedDetails.find((d) => d.TradeEntryDetailId?.toString() === urlDetailId)
        : undefined
      if (targetDetail) {
        setActiveTabId(targetDetail.LocalTradeEntryDetailId)
      }
  ...
  function getUpdatedQuantities(
    ...
    const startDate = dayjs(newEffectiveDates[0])
    const endDate = dayjs(newEffectiveDates[1])
    const months: string[] = []
  ...
  const closeTab = (tabToClose: string) => {
    if (!tabToClose.includes('duplicate')) {
      setDetails((prev) =>
        prev.map((d) => (d.LocalTradeEntryDetailId === tabToClose ? { ...d, isOpen: false } : d))
      )
    }
    if (tabToClose === activeTabId) {
      setActiveTabId('0')
    }
  }
  ...
  const createFormula = (variables: Array<{ VariableName: string }>, type: string) => {
      ...
      for (let i = 0; i < sortedGroupDetail?.length; i++) {
        ...
        if (curr && next) {
          const hasInvalidDates =
            dayjs(next.FromDateTime).isSame(curr.FromDateTime) ||
            dayjs(next.ToDateTime).isSame(curr.ToDateTime) ||
            dayjs(next.FromDateTime).isSameOrBefore(curr.ToDateTime)
          if (hasInvalidDates) {
  ...
  const saveContract = async (makeActive?) => {
    const detailsToSend = details.map((detail) => {
      const detailNetOrGross = metadata?.NetOrGrossTypeList.find((item) => item.Value === detail.NetOrGrossCvId?.toString())
      ...
      return {
        ...
        FromDateTime: dayjs(detail?.EffectiveDates[0])?.startOf('day').format(dateFormat.ISO),
        ToDateTime: dayjs(detail?.EffectiveDates[1])?.endOf('day').format(dateFormat.ISO),
        ...
        Quantities: hasQuantities
          ? detail?.Quantities?.filter((q) => q.IsActive).map((q) => {
              return {
                IsActive: q.IsActive,
                Quantity: q.Quantity,
                QuantityDateTime: dayjs(q.QuantityDateTime).format(dateFormat.ISO),
                TradeEntryQuantityId: q?.TradeEntryQuantityId || 0,
              }
            })
          : [],
        Prices: detail?.Prices?.map((p) => {
          let FormulaVariables: Array<{ VariableName: string } & Record<string, unknown>> = []
          ...
          return {
            ...
            FromDate: dayjs(p.FromDate).startOf('day').format(dateFormat.ISO),
            ToDate: dayjs(p.ToDate).endOf('day').format(dateFormat.ISO),
        ...
        ValuationCalendarId: detail?.ValuationCalendarId ? parseInt(detail.ValuationCalendarId) : 0,
      }
    })
    ...
    const objectToSend = {
      ...
      Contract: {
        ...
        FromDateTime: dayjs(header?.EffectiveDates?.[0]).startOf('day').format(dateFormat.ISO),
        ToDateTime: dayjs(header?.EffectiveDates?.[1]).endOf('day').format(dateFormat.ISO),
        ...
        TradeEntryDateTime: dayjs(header?.TradeEntryDateTime).startOf('day').format(dateFormat.ISO),
          ...
          if (result?.Data?.TradeEntryId) {
            if (header?.TradeEntryId) {
              queryClient.invalidateQueries({
                queryKey: contractQueryKeys.details
              })
            } else {
              queryClient.invalidateQueries({
                queryKey: contractQueryKeys.details
