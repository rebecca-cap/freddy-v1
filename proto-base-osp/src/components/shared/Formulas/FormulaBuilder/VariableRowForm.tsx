...
import { validateInstrument, validatePriceType } from './utils/formHelpers'
...
export function VariableRowForm({ variable, variableIndex, metadata, name, remove, form, data, groupName }) {
  ...
  const priceTypeOptions = useMemo(() => {
    if (!selectedPublisherId) {
      return variable?.PriceTypeCvId ? (metadata?.PriceTypeList || []).map(toAntOption) : []
    }
    const publisherId = +selectedPublisherId || 0
    const publisherTypes = metadata?.PublisherPriceTypes[publisherId] || []
    const validOptions = publisherTypes.map(toAntOption)
    // If the current value isn't valid for this publisher, include it as a disabled
    // option so the Select can still render its label but the user can't re-pick it.
    const currentTypeId = variable?.PriceTypeCvId?.toString()
    if (currentTypeId && !publisherTypes.some((pt) => pt.Value == currentTypeId)) {
      const fullMatch = metadata?.PriceTypeList?.find((pt) => pt.Value == currentTypeId)
      if (fullMatch) {
        return [...validOptions, { value: fullMatch.Value, label: fullMatch.Text, disabled: true }]
      }
    }
    return validOptions
  }, [selectedPublisherId, metadata?.PublisherPriceTypes, metadata?.PriceTypeList, variable?.PriceTypeCvId])
  ...
  const handleAOSave = (formValues) => {
    const groupFieldValue = form.getFieldValue(['Groups', groupName, name])
  ...
  return (
    <Horizontal className='px-3 py-2 border-bottom bg-1'>
      ...
      <Vertical flex={1} className='mr-3'>
        ...
        >
          <Select allowClear showSearch placeholder='Type' popupMatchSelectWidth={false} options={priceTypeOptions} />
        </Form.Item>
      </Vertical>
      <Vertical flex={1} className='mr-3'>
