...
export const defaultDateFormatter = (params) => (params?.value ? moment(params?.value).format('L LTS') : '')
...
function getDefaultSorter(column: EntityViewColumn) {
  if (isDateOrTimeColumn(column)) {
    return (a, b) => {
      const ma = a ? moment(a) : moment().set('year', 1900)
      const mb = b ? moment(b) : moment().set('year', 1900)
...
export const valuationStatusCellStyleFn = (params) => {
  ...
  let cols = [
    ...response.EntityView.EntityViewColumns.filter((c) => c.IsVisible).map((column) => {
      const def = {
        ...
        ...(isDateOrTimeColumn(column) && { filter: 'agDateColumnFilter' }),
...
export const eagerlyUpdateRowData = <T extends Record<string, any>>(
  ...
  queryClient.setQueriesData<{ TotalRecords: number; Data: T[]; Query: {} }>([cacheKey], (cache) => {
    if (!cache) return
