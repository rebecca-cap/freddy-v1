...
export function PromptProcessedVolume({ strokeColor, chartFilters, onFiltersChange, selectOptions }) {
  ...
  if (isFetching) {
    return (
      <Horizontal className='bg-1' verticalCenter horizontalCenter style={{ height: '15rem' }}>
        <Texto category='h3'>
          <LoadingOutlined />
        </Texto>
      </Horizontal>
  ...
  return (
    <Horizontal className='bg-1' verticalCenter horizontalCenter style={{ height: '13rem' }}>
      <Vertical>
        <Horizontal
        ...
        {!chartData?.length && (
          <Horizontal className='bg-3 m-4 p-4'>
            <NothingMessage
              title='No data found'
              message={`There was no data found for ${chartFilters?.ProcessedPromptVolume?.label?.toLowerCase()}`}
            />
          </Horizontal>
        )}
      </Vertical>
