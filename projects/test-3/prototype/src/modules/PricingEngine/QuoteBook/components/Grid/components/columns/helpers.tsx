...
import { DirtyUpdateField } from '@modules/PricingEngine/QuoteBook/type.schema'
...
export const getDirtyCellStyle = (row: Quote, originalRows: Quote[] | undefined, fieldName: string) => {
  const adjustedToday = row.AdjustmentUpdatedDateTime
    ? moment(row.AdjustmentUpdatedDateTime).isSame(moment(), 'day')
    : false

  const spreadOverrideToday = row.SpreadOverrideUpdatedDateTime
    ? moment(row.SpreadOverrideUpdatedDateTime).isSame(moment(), 'day')
    : false

  const recentlyPublished =
    row.LatestQuoteDate && row.AdjustmentUpdatedDateTime
      ? moment(row.LatestQuoteDate).isAfter(row.AdjustmentUpdatedDateTime, 'second')
      : false
  ...
  if (needsDirtyCellStyle) {
    return { backgroundColor: '#E2EBF8', opacity: 1 }
  }
}
...
export function getApplicableMarketMoveValueFromRowData(data: Quote) {
