...
export function dtnSupplierColumnDefs({ metadata, canWrite }: IGetColumnDefProps) {
  return [
    {
      editable: false,
      field: 'SourceValue',
    ...
    {
      field: 'TargetCounterPartyId',
      headerName: 'Supplier',
      cellEditor: 'SearchableSelect',
      isBulkEditable: canWrite,
      bulkCellEditor: BulkSelectEditor,
      bulkCellEditorParams: {
        options: metadata?.CounterParties,
        propKey: 'TargetCounterPartyId',
        selectEditorProps: {
          showSearch: true,
          allowClear: true,
        },
      ...
      valueGetter: (params) =>
        metadata?.CounterParties?.find(
          (option) => option.Value?.toString() === params?.data?.TargetCounterPartyId?.toString()
        )?.Text,
      cellRenderer: ({ value }) => {
        if (value) {
          return (
    ...
    },
    {
      ...
      bulkCellEditor: BulkSelectEditor,
      bulkCellEditorParams: {
        options: metadata?.PricePublishers,
        propKey: 'TargetPricePublisherId',
        selectEditorProps: {
          showSearch: true,
          allowClear: true,
        },
      ...
      valueGetter: (params) =>
        metadata?.PricePublishers?.find(
          (option) => option.Value?.toString() === params?.data?.TargetPricePublisherId?.toString()
        )?.Text,
      cellRenderer: ({ value }) => {
        if (value) {
          return (
