...
export function QuoteBookPage() {
  ...
  return (
    // ScoutProvider is hoisted to AuthenticatedRoute (wraps <Outlet/>) so
    // Scout state persists across navigation. This page is now a plain
    // consumer — Panel / Mini / intro bubble / shortcuts / grid wiring stay
    // mounted here (still descendants of the hoisted provider). The toast host
    // + global timer bar render once at the shell.
    <>
      <Vertical className='flex-1 vertical-flex scout-push-host'>
        <QuoteBookHeader
          metadata={metadata}
