...
export function PriceInfoColumns({
  ...
  return {
    ...
    children: [
      ...
      Product(),
      Commodity(),
      ...getReportingAttributesColumns(originalRows),
    ],
  }
}
const ContractId = ({}) => ({
          ...
          <Tooltip
            title={
              params?.data?.LatestQuoteDate
                ? `Published ${moment(params?.data?.LatestQuoteDate).format(dateFormat.DATE_TIME)}`
                : 'No Date Available'
            }
          >
            <BBDTag
              error={moment(params.data.LatestQuoteDate).isBefore(moment().subtract(3, 'd'))}
              success={
                params?.data?.LatestQuoteDate &&
                moment(params.data.LatestQuoteDate).isAfter(moment().subtract(2, 'h')) &&
                moment(params.data.LatestQuoteDate).isBefore(moment().add(2, 'h'))
              }
              style={{ width: 30 }}
            >
              <Horizontal verticalCenter>
...
const LastPublication = () => ({
  ...
  valueGetter: (params) => {
    return params?.data?.LatestQuoteDate ? moment(params.data.LatestQuoteDate) : ''
  },
  filterParams: {
    buttons: ['reset'],
  },
...
const LastEffective = () => ({
  ...
  valueFormatter: ({ value }) => (value ? moment(value).format(dateFormat.SHORT_DATE_YEAR_TIME) : ''),
  valueGetter: (params) => {
    return params?.data?.LatestQuoteEffective ? moment(params.data.LatestQuoteEffective) : ''
  },
  filterParams: {
    buttons: ['reset'],
  },
