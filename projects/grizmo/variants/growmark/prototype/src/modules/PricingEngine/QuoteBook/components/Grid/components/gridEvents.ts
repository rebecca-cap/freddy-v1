...
export function updateAssociatedSpreadRows(
  singleQuote: Quote,
  refs: any,
...
export function getLastSaveDate({ quotes }: GetLastSaveDateProps) {
  ...
  const initialDate: moment.Moment | null = null

  const maxDate: moment.Moment | null | undefined = quotes?.reduce<moment.Moment | null>((date, row) => {
    if (!row.AdjustedDate) return date

    const currentDate = moment(row.AdjustedDate)
