...
export function PricingInformation({ selectedSubtype, form, orderEntryInfo }) {
  ...
  return (
    <Vertical flex={1} className='pricing-information'>
      <Texto category='h5' className='mt-3 mb-3' appearance='secondary'>
        Pricing Information
      </Texto>
      ...
      {selectedSubtype.ContractPricingMethodMeaning !== 'DeliveryPeriod' && (
        <Horizontal className='px-3 py-2' verticalCenter fullHeight>
          ...
          <Texto category='h4'>${fmt.decimal(form.getFieldValue('Price'))}</Texto>
        </Horizontal>
      )}
      {showOverridePrice && (
        <Horizontal className='px-3 py-2' verticalCenter fullHeight>
      ...
      {form.getFieldValue('PricingStrategy') === 'Bid' && selectedSubtype.AllowBid && (
        <>
          ...
          <Horizontal className='px-3 py-2' verticalCenter>
            <Texto style={{ flex: 1 }}> EXPIRATION DATE: </Texto>
            <Form.Item
              name='BidExpiration'
              style={{ flex: 2, textAlign: 'right' }}
              rules={[{ required: true, message: 'Date selection is required' }]}
              initialValue={moment(orderEntryInfo?.Data?.SelectedItems[0].Defaults.DefaultBidExpiryDateTime)}
            >
              <DatePicker
                size='middle'
                ...
                disabledDate={(date) =>
                  date.isAfter(moment(orderEntryInfo?.Data?.SelectedItems[0].Constraints.MaximumBidExpiration))
                }
