...
export function RelatedProducts({ metadata, products, selectedRows, createUpdateProductManagementMutation, canWrite }) {
  const selectedProduct = selectedRows[0]
  const relatedProducts = selectedProduct?.MarketPlatformAdditionalProducts?.map((id) => id.toString())

  useEffect(() => {
    const productSelectionList = metadata?.Data?.ProductGroups.map((group) => {
      const groupProducts = products?.Data?.filter(
        (product) => product.ProductGroupId && product.ProductGroupId.toString() === group.Value
      ).map((product) => {
        return { ...product, selected: relatedProducts?.includes(product.ProductId.toString()) }
      })
      ...
      return { ...group, selected: allItemsSelected, collapsed: false, groupProducts }
    })
  ...
  const handleGroupChange = (group) => {
    const updateProductGroups = productGroups.map((item) => {
      const searchResultGroupProducts = searchResults
        ...
        ?.groupProducts?.map((searchProduct) => searchProduct.ProductId)

      if (item.Value === group.Value) {
        const updatedSelectedProducts = item.groupProducts.map((product) => {
          return {
            ...product,
            selected: searchResultGroupProducts.includes(product.ProductId) ? !item.selected : item.selected,
          }
        })
    ...
    setProductGroups(updateProductGroups)
    setSearchResults(searchProducts(searchString, updateProductGroups))
    return saveRelatedProducts(updateProductGroups)
  }
  ...
  const saveRelatedProducts = (updateProductGroups) => {
    const selectedProductIds: string[] = []

    updateProductGroups.forEach((g) => {
      g.groupProducts.forEach((p) => {
        if (p.selected) {
          selectedProductIds.push(p.ProductId)
        }
      })
    })
    ...
    const updatedProduct = {
      ...selectedProduct,
      MarketPlatformAdditionalProducts: [...selectedProductIds],
    }
    return createUpdateProductManagementMutation.mutateAsync([updatedProduct])
  }
  ...
  return (
    <Vertical style={{ minHeight: 'auto' }}>
      <Horizontal className='px-4 pt-3 py-2 bg-2'>
        <Texto category='h3'>Related Products</Texto>
      </Horizontal>
      <Horizontal>
        <Input
        ...
        <Vertical verticalCenter>
          <Horizontal verticalCenter horizontalCenter>
            <NothingMessage
              ...
              message='Select a product to manage the related products for it.'
            />
          </Horizontal>
        </Vertical>
      )}
