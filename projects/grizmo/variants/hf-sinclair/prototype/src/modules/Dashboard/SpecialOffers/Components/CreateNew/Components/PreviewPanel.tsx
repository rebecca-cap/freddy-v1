...
export function PreviewPanel({
  ...
  const getPricingStrategyValue = () => {
    ...
    return (
      <Texto className={'text-wrap-whitespace text-14'}>
        {preface}: {fmt.currency(pricingStrategy)}/gal
      </Texto>
    )
  }
  ...
  const getTimingWindowsValue = () => {
    if (
      ...
      return ''
    const VisStartDateTime = moment(selectedVisibilityWindowStart).set({
      hour: moment(visibilityStartTime, 'HH:mm').hour(),
      minute: moment(visibilityStartTime, 'HH:mm').minute(),
    })
    const VisEndDateTime = moment(selectedVisibilityWindowEnd).set({
      hour: moment(visibilityEndTime, 'HH:mm').hour(),
      minute: moment(visibilityEndTime, 'HH:mm').minute(),
    })
    const PickStartDateTime = moment(selectedPickupWindowStart).set({
      hour: moment(pickupStartTime, 'HH:mm').hour(),
      minute: moment(pickupStartTime, 'HH:mm').minute(),
    })
    const PickEndDateTime = moment(selectedPickupWindowEnd).set({
      hour: moment(pickupEndTime, 'HH:mm').hour(),
      minute: moment(pickupEndTime, 'HH:mm').minute(),
    })
    return (
      <>
