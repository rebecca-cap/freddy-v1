...
function GateKeeper({ setDefaultHeaders, setActiveLink }) {
  ...
  // <ScoutProvider> is hoisted ABOVE the RouterProvider so it is mounted once
  // for the whole authenticated session and NEVER remounts on SPA navigation —
  // this is what keeps the single global timer continuous and all thread/alert
  // state alive across pages (TMR-3 / ALR-5). The cross-page surfaces (bottom
  // prototype timer bar + toast host) render here at the shell; the chrome name
  // tag / drawer cards and the QuoteBook panel are all descendants via the
  // router, so useScout() resolves everywhere.
  return (
    <ScoutProvider>
