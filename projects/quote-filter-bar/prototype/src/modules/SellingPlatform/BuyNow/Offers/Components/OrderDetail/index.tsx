...
import { PriceExpiredModalContent } from '@modules/SellingPlatform/BuyNow/sharedComponents/PriceExpiredModalContent'
...
import { CreateOrder } from '../CreateOrder/OffersCreateOrder'
import { ConfirmOrder } from './ConfirmOrder'
import { OrderFooter } from './OrderFooter'
import { OrderHeader } from './OrderHeader'

type RefetchFn<TData = unknown, TError = unknown> = (
...
export function OrderDetailPane({
  ...
  const clearOrderState = () => {
    refetchItemsAvailableForOrder().then(() => {
      form.resetFields()
  ...
  const handleNextStep = () => {
    if (orderStep === 'create') {
      ...
      const laterStartDate = moment().isAfter(moment(startDate)) ? moment().add(1, 'minute') : startDate
  ...
  return (
    <Vertical className='order-form'>
      <Vertical flex='none' height='auto'>
        <OrderHeader form={form} showOrderTimer={false} showTimeRemaining={true} />
      </Vertical>
      <Vertical height='auto' style={{ position: 'relative' }}>
        {isPriceExpired && form.getFieldValue('Type') !== 'bid' && (
          <PriceExpiredModalContent
            selectedItemMeta={selectedItemMeta}
            setIsPriceExpired={setIsPriceExpired}
            refreshTradeEntryData={refreshEntryData}
            refreshItemsAvailableForOrder={refetchItemsAvailableForOrder}
          />
        )}
        <Vertical>
          <OrderStepComponent
