...
export function OrderExpirationModal({ isPriceExpired, handleRefresh, tradeTimer, orderEntryInfo }) {
  ...
  return (
    <Horizontal className='justify-sb'>
      <Horizontal flex={5} style={{ gap: 10 }}>
      ...
      <Horizontal flex={2} horizontalCenter style={{ marginRight: -25 }}>
        <PriceExpiredModal visible={isPriceExpired} handleRefresh={handleRefresh} />
...
function PriceExpiredModal({ visible, handleRefresh }) {
  return (
    <Modal className='expired-modal' visible={visible} footer={null} closable={false} centered>
      <Vertical style={{ width: '100%' }}>
        <Horizontal className=' bordered bg-1'>
          <Vertical className='m-4'>
            <Horizontal className='items-center' style={{ gap: 5 }} verticalCenter>
