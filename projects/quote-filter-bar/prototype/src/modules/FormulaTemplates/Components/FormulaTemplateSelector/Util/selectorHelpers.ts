...
import { ContractManagementMetadata, FormulaVariable } from '@modules/ContractManagement/api/types.schema'
...
import { mapFormulaVariableToTemplateVariable } from '@modules/FormulaTemplates/Util/formHelpers'
 ...
 */
export function isBlankFormulaVariable(variable: FormulaVariable): boolean {
...
export function mapTemplateVariableToFormulaVariable(
  ...
  return {
    ...
    PricePublisherName: '',
    PriceTypeCvId: templateVar.PriceTypeCvId ?? null,
...
export function getUpdatedProvision(provisionToEdit, selectedTemplate, targetGroupIndex, metadata): any {
  ...
  const newName = currentName ? `${currentName} + ${selectedTemplate.Name}` : selectedTemplate.Formula
