...
export function BidUpdateFields({
      ...
      validator: (_, value) => {
        ...
        if (value < constraints.VolumeConstraintMinimumVolume) {
          return newPromise.reject(
            new Error(`Volume must be at least ${fmt.decimal(constraints.VolumeConstraintMinimumVolume, 0)} gal(s)`)
          )
        }
        if (value > constraints.VolumeConstraintMaximumVolume) {
          return newPromise.reject(
            new Error(`Volume cannot exceed max of ${fmt.decimal(constraints.VolumeConstraintMaximumVolume, 0)} gal(s)`)
          )
        }
  ...
  const getQuantityFieldStatus = useMemo(
    () => (getStatus?: boolean) => {
      ...
      if (value && value < constraints.VolumeConstraintMinimumVolume) {
        return getStatus
          ? 'error'
          : `Volume must be at least ${fmt.decimal(constraints.VolumeConstraintMinimumVolume, 0)} gal(s)`
      }
      if (value && value > constraints.VolumeConstraintMaximumVolume) {
        return getStatus
          ? 'error'
          : `Volume cannot exceed max of ${fmt.decimal(constraints.VolumeConstraintMaximumVolume, 0)} gal(s)`
      }
  ...
  if (!showDateOverrideFields) {
    return (
      <Horizontal verticalCenter className='m-1' justifyContent='flex-end' style={{ gap: '15px' }}>
