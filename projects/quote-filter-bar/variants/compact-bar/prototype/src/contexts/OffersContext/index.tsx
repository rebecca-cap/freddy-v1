...
import type { AllSpecialOffersDisplayModel, CreditData, SubmitOrderRequest } from '@modules/SellingPlatform/BuyNow/Offers/Api/types.schema'
...
export const OffersProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const { getOrderEntryDataBySpecialOfferId, submitOrder } = useOffers()
  ...
  const handleSubmitOrder: OffersContextValue['handleSubmitOrder'] = ({ clearOrderState }) => {
    ...
    submitOrderMutation.mutateAsync(trade)
      .then((response: any) => {
  ...
  const refreshEntryData = () => {
    ...
    getOrderEntryDataBySpecialOfferId(selectedOffer?.ItemKey)
      ...
      .catch((err: any) => {
        NotificationMessage('Error loading order data', err?.message ?? 'Unknown error')
      })
