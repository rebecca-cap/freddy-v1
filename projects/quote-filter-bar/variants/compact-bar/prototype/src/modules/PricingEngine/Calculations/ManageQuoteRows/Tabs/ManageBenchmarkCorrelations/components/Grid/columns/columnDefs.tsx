...
const PriceEffective = () => ({
  ...
  valueGetter: ({ data }) => {
    ...
    if (!value) return ''
    const start = moment(value, 'HH:mm:ss').format(dateFormat.FULL_TIME)
    const end = moment(value, 'HH:mm:ss').subtract(1, 'minutes').format(dateFormat.FULL_TIME)
    return `${start} - ${end}`
  },
  cellRenderer: ({ value }) => {
    const startOffset = moment(value, 'HH:mm:ss').format(dateFormat.FULL_TIME)
    const endOffset = moment(value, 'HH:mm:ss').subtract(1, 'minutes').format(dateFormat.FULL_TIME)
    return (
