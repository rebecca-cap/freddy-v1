...
export function DateChangeConfirmContent({
  ...
  const pricesWithUpdatedDates = useMemo(() => {
    ...
    return prices.map((price) => ({
      ...price,
      FromDate: moment(price.FromDate).add(useStartOverride ? startValue : dateDiffs[0], 'days'),
      ToDate: moment(price.ToDate).add(useEndOverride ? endValue : dateDiffs[1], 'days'),
    }))
  ...
  const errorPriceDates = useMemo(() => {
    ...
    detailsWithUpdatedDates?.forEach((detail) => {
      ...
      for (let i = 1; i < detail.Prices.length; i++) {
        ...
        if (moment(previousPrice.ToDate).isSameOrAfter(moment(currentPrice.FromDate))) {
          hasOverlap = true
          break
        }
      }
      ...
      detail.Prices.forEach((price, index) => {
        ...
        if (moment(price.FromDate).isAfter(moment(effectiveDates[1]))) {
          errorList.push('From Date is after Contract End Date')
        }
        if (moment(price.ToDate).isBefore(moment(effectiveDates[0]))) {
          errorList.push('To Date is before Contract Start Date')
        }
        if (moment(price.FromDate).isAfter(moment(price.ToDate))) {
