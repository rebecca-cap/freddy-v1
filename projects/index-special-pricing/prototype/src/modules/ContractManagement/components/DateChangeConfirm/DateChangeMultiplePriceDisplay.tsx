  ...
  const { startMinValue, startMaxValue, endMinValue, endMaxValue } = useMemo(() => {
    if (effectiveDates) {
      ...
      if (relevantPrices.length > 0) {
        const fromDates = relevantPrices.map((price) => dayjs(price.FromDate))
        const toDates = relevantPrices.map((price) => dayjs(price.ToDate))

        const earliestFromDate = dayjs.min(fromDates)
        const latestToDate = dayjs.max(toDates)

        if (!earliestFromDate || !latestToDate) {
      ...
      <div style={{ overflowY: 'scroll', maxHeight: '300px' }}>
        {pricesWithUpdatedDates?.length > 0 &&
          pricesWithUpdatedDates.map((price) => {
            return (
              <Horizontal key={`priceError-${price.LocalTradeEntryPriceId}`} verticalCenter gap='5px'>
                ...
                <Horizontal verticalCenter flex={1}>
                  ...
                  <Texto>{dayjs(price.FromDate).format(dateFormat.DATE)}</Texto>
                </Horizontal>
                <Horizontal verticalCenter flex={1}>
                  <Texto className='mr-2'>To:</Texto>
                  <Texto>{dayjs(price.ToDate).format(dateFormat.DATE)}</Texto>
                </Horizontal>
              </Horizontal>
            )
          })}
