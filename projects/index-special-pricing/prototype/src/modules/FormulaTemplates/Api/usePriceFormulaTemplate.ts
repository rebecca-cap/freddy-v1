...
export function usePriceFormulaTemplate(
  endpoints: FormulaTemplateEndpoints = PEFormulaTemplateEndpoints,
  ...
  const useSaveFormulaTemplateMutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: [endpoints.getAllFormulaTemplates] })
      if (onSaveInvalidateKey) {
        await queryClient.invalidateQueries({ queryKey: [onSaveInvalidateKey] })
      }
    ...
    onError: (error: any) => {
      NotificationMessage(
        'Unable to save',
        error?.json?.Validations?.[0]?.Message || error?.json?.message || 'Error on save formula',
        true
      )
    },
  })
