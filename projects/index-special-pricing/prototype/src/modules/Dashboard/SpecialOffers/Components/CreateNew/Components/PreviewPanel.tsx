...
import { Texto, Vertical, addCommasToNumber } from '@gravitate-js/excalibrr'
import {
...
interface PreviewPanelProps {
  ...
  isAuction: boolean
  /** Quick-create opt-in: show the offered volume under the product/location line. */
  totalVolume?: number
  /** Replaces the default "What customers will see" subtitle (quick-create passes the full-setup link). */
  subtitle?: ReactNode
}
  ...
  const getProductValue = () => {
    ...
    return (
      <>
        ...
        {isDefinedAndNotNull(totalVolume) && (
          <Texto className={'text-14 text-muted'}>
            {addCommasToNumber(totalVolume)} {item?.UnitOfMeasureSymbol ?? 'gal'}
          </Texto>
        )}
      </>
    )
  ...
  return (
    <Vertical>
      <Vertical className={'mb-4'}>
        ...
        {subtitle !== undefined ? subtitle : <Texto className={'text-14 text-muted'}>What customers will see</Texto>}
      </Vertical>
      {items.map((item) => (
        <Vertical
