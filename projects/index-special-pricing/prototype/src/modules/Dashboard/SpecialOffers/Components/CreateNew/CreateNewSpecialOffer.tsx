...
import type { FullSetupCarry } from '@modules/Dashboard/SpecialOffers/utils/Constants/PresetConstants'
...
interface CreateNewSpecialOfferProps {
  ...
  /** Values carried over when the user switches from quick-create → full setup. */
  initialPrefill?: FullSetupCarry
  metadata?: SpecialOfferMetadataResponseData
  /** Render the wizard body without its own Drawer so a host can show it as an in-drawer view. */
  renderChromeless?: boolean
}
...
export function CreateNewSpecialOffer({
  ...
  useEffect(() => {
    if (isShowingCreateNew && metadata) {
      // The quick-create → full-setup carry seeds every field itself; skip blank defaults
      // so we don't clobber the values the user already entered.
      if (initialPrefill) {
  ...
  }, [isShowingCreateNew, metadata, priorOffer, initialPrefill])

  // Quick-create → full-setup carry-over: seed template, category, every field value, and
  // any index-pricing config, then land on the Products step and trigger grid re-selection.
  useEffect(() => {
    if (!isShowingCreateNew || !metadata || !initialPrefill) return
    if (initialPrefill.category) setSelectedCategory(initialPrefill.category)
    if (initialPrefill.templateId) setSelectedTemplateId(initialPrefill.templateId)
    form.setFieldsValue(initialPrefill.values)
    if (initialPrefill.indexPricingData) {
      setIndexPricingData(initialPrefill.indexPricingData)
      form.setFieldsValue({ IndexPrice: initialPrefill.indexPricingData })
    }
    setCurrentStep(1)
    setPrefillFocusToken((t) => t + 1)
  }, [isShowingCreateNew, metadata, initialPrefill])
  ...
  useEffect(() => {
    if (!isShowingCreateNew || prefillFocusToken === 0 || !(priorOffer || initialPrefill)) return
  ...
  }, [prefillFocusToken, isShowingCreateNew, priorOffer, initialPrefill])
  ...
  // Chromeless: render the wizard body directly so a host (quick-create) can show it as an
  // in-drawer view instead of stacking a second drawer.
  if (renderChromeless) return content
