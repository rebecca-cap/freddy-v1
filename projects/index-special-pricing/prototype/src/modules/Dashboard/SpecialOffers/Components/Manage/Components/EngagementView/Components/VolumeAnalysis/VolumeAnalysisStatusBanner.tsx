...
export function VolumeAnalysisStatusBanner({ specialOfferDetails }: VolumeAnalysisStatusBannerProps) {
  ...
  const uomSymbol = specialOfferDetails?.Data?.OfferInfo?.UnitOfMeasureSymbol ?? defaultUnitOfMeasureSymbol
  ...
  return (
    <Horizontal className={'volume-analysis-status-banner-container'} verticalCenter>
      ...
      <Texto>
        Accepting all pending bids would exceed capacity by {addCommasToNumber(exceedAmount)} {uomSymbol}(s). You can
        accept up to {addCommasToNumber(remainingCapacity)} {uomSymbol}(s) more.
      </Texto>
    </Horizontal>
  )
}
