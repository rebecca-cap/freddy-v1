...
interface SelectInvitationDateProps {
  ...
  /** Class applied to the portaled dropdown panels (rounds them in the quick-create drawer). */
  ...
  return (
    <Vertical className={'p-4 bordered mb-4 border-radius-5'}>
      ...
      <Horizontal className={'mb-2'} verticalCenter style={{ gap: 8 }}>
        <Form.Item name={'SendInvitesOnCreate'} valuePropName='checked' className={'mb-0'}>
          <Checkbox
            onChange={(check) => {
      ...
      <Horizontal className={'mb-2'} verticalCenter style={{ flexWrap: 'wrap', gap: 8 }}>
        <Texto className={'text-14'} style={{ minWidth: 140 }} appearance={sendOnCreate ? 'hint' : 'default'}>
          Invite Trigger Date:
        </Texto>
        <Form.Item className={'mb-0'} name={'InviteTriggerDate'} rules={[{ validator: validateTriggerDate }]}>
          <DatePicker
        ...
        <Texto className={'text-14'} appearance={sendOnCreate ? 'hint' : 'default'}>
          at
        </Texto>
        <Form.Item className={'mb-0'} name={'InviteTriggerTime'} rules={[{ validator: validateTriggerTime }]}>
          <TimeSelect disabled={sendOnCreate} popupClassName={popupClassName} quickStarts={timeQuickStarts} />
        </Form.Item>
        {selectedTimezone && <BBDTag>{selectedTimezone}</BBDTag>}
      </Horizontal>
      <Horizontal className={'mb-2'} verticalCenter style={{ flexWrap: 'wrap', gap: 8 }}>
        <Texto className={'text-14'} style={{ minWidth: 140 }}>
          Time Zone:
        </Texto>
        <Form.Item className={'mb-0'} name={'TimeZoneId'} rules={[{ required: true, message: 'Time zone is required' }]}>
          <Select
            showSearch
