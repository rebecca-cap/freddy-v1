...
export function ProductListings({ productListings, isFetching }) {
  ...
  return (
    <Vertical flex={1}>
      ...
      {!groupedProductListings?.length ? (
        <Horizontal className='p-4 bg-1 mt-5 header-container' horizontalCenter style={{ height: '73vh' }}>
          <Texto category='p1' style={{ color: 'var(--gray-600)' }}>No product listings data</Texto>
        </Horizontal>
      ) : (
  ...
  return (
    <Horizontal className='bg-2 px-1 py-2 justify-sb' verticalCenter width='100%'>
      ...
      <Horizontal flex={1} justifyContent={'flex-end'}>
        <Texto category='p2'>{fmt.currency(product?.Price)}</Texto>
      </Horizontal>
    </Horizontal>
  )
}
    ...
    <Horizontal className='product-listing'>
      <Collapse
        ...
        items={[
          {
            key: '1',
            label: <ProductListingHeader listing={listing} />,
            children: (
              <Vertical>
