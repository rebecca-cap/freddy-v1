import { BarChartOutlined, FileOutlined, FunctionOutlined, LinkOutlined } from '@ant-design/icons'
...
import { isDefined, toAntOption } from '@utils/index'
...
export const createColumnDefs = ({
  ...
  mutationRef: MutableRefObject<
    UseMutationResult<QuoteConfigurationCreateResponse, unknown, QuoteConfigurationMappingUpsertPayload, unknown>
  >
  canWrite: boolean
}): ColDef[] => {
  return [
    CompetitorAssociationCount(),
    Id(),
...
const CompetitorAssociationCount = () => ({
...
const Active = (canWrite, mutationRef) => ({
  isBulkEditable: true,
  ...
  editable: false,
  filterParams: {
    valueFormatter: (params) => (params.value == 100 ? 'Enabled' : 'Disabled'),
  },
  cellRenderer: (params) => {
    return (
      <Switch
...
const ProductOverride = (metadata) => ({
...
const PriceEffective = (metadata) => ({
  field: 'PricePeriodStartOffset',
  headerName: 'Price Effective',
  isBulkEditable: true,
  bulkCellEditor: BulkSelectEditor,
  bulkCellEditorParams: {
    options: metadata?.Data?.PricePeriodStartOffsets?.map(toAntOption) ?? [],
...
const Strategy = (metadata, canWrite: boolean) => ({
...
const getStategyOptions = (metadata, quoteConfigurationId) => [
  ...
  ...(metadata?.Data?.Benchmarks?.filter((bo) => !quoteConfigurationId || bo.GroupingValue == quoteConfigurationId).map(
    (item) => ({
      ...
      }, "StrategyQuoteBenchmarkId":${item.Value}}`,
      label: item.Text,
    })
  ) ?? []),
]

const ReferenceStrategy = (metadata, canWrite: boolean) => ({
...
const AutoPublishType = (metadata) => ({
...
const MarketMoveMarker = (metadata) => ({
  ...
  isBulkEditable: ({ data }) => data?.UsesMarketMove,
  bulkCellEditor: BulkSelectEditor,
  bulkCellEditorParams: {
    getChanges: (value: string | undefined) => ({
...
const UseMarketMove = () => TrueFalseBulkEditableColumn('UsesMarketMove', 'Use Market Move', true)

const Actions = (canWrite, mutationRef) => {
  if (canWrite) {
    return [
      {
        colId: 'actions',
