...
import { GetUserInfoResponse, SecurityContext } from '@api/useCredential/responseTypes'
...
export type PermissionsObject = {
  [key: string]: PermissionActions | NestedPermissions | SuperUserPermission
  MarketPlatform?: {
...
export function UserProvider({ children, setDefaultHeaders }: UserProviderProps) {
  ...
  const { useUserInfoQuery, getImpersonationOptionsQuery } = useCredential()
  ...
  const { data: availablePageKeys, isLoading } = useQuery(
    ['scopes'],
    async () => {
      const data = await api.post('Application/Menu/GetMenuItemsByQuery', { MenuName: 'MainMenu' })
