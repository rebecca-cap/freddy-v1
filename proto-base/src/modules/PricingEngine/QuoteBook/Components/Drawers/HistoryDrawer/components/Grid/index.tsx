...
import type { QuoteHistoryResponse } from '@modules/PricingEngine/QuoteBook/Components/Drawers/HistoryDrawer/api/schema.types'
...
export function QuoteHistoryGrid({
  ...
  const controlBarProps = useMemo(() => {
    const locationLabel =
      quoteHistoryHeaderInfo?.LocationName ??
      (quoteHistoryHeaderInfo?.OriginLocationName
        ? `${quoteHistoryHeaderInfo.OriginLocationName}-${quoteHistoryHeaderInfo.DestinationLocationName ?? ''}`
        : '')
    const fileName = `${locationLabel}-${quoteHistoryHeaderInfo?.ProductName ?? ''}-${
