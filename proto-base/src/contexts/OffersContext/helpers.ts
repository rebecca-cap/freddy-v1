...
export const createSelectList = (keyList?: Record<string, string>, selected?: string) =>
...
export const mapEntryDataToContext = (
  ...
  const meta: OffersSelectedItemMetadata = {
    ...
    BidExpiration: moment(tradeSelectedItem?.Defaults?.DefaultBidExpiryDateTime),
    ...
    Type: tradeSelectedItem?.SpecialOfferData?.PricingMechanism === 'Bid' ? 'bid' : 'market',
    CreditData: {
...
export const buildSubmitOrderPayload = (
  ...
  const selectedAdditionalItems =
    meta.AdditionalItems?.filter((p) => pending?.SelectedItems?.map((i: any) => i.key).includes(p.key)).map((x) => ({
      ItemKey: x.ItemKey,
    })) ?? []
  ...
  const payload: SubmitOrderRequest = {
    ...
    BidExpiry: isBid ? moment(pending?.BidExpiration)?.format(dateFormat.ISO_V2) : undefined,
    Notes: pending?.Notes,
    ...
    OverrideStartDate: pending?.OverrideStartDate ? moment(pending?.OverrideStartDate).format(dateFormat.ISO_V2) : null,
    OverrideEndDate: pending?.OverrideEndDate ? moment(pending?.OverrideEndDate).format(dateFormat.ISO_V2) : null,
  }
